...
def drug_filter_from_dict(drug_cid_dict, ref_drug):
    drugs = []
    cids = []
    for drug, cid_list in drug_cid_dict.items():
    ...
    df = pd.DataFrame({'Drug': drugs, 'Cid': cids})
...
def function_call(drug_list, file, cancer_name):    
    ...
    if drug_list is None:
        ...
        drug_list = [line.strip() for line in drug_list.readlines() if line.strip()]
        drug_cid = {}
        ...
        drug_lists = pd.DataFrame(drug_list, columns=['Drug'])
        drug_df = drug_filter_from_dict(drug_cid, drug_lists)
        approved_drugs = remove_drug(drug_df, cmap)
        approved_drugs['Drug'] = approved_drugs['Drug'].str.lower()
        approved_drugs = approved_drugs.groupby('Drug')['Cid'].apply(lambda x: ','.join(x)).reset_index()
...
def overlap_analysis_cmps(drug_list = None, gtcx_file_path = None, cancer = None):
